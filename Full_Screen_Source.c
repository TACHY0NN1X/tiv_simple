...
int main( int argc , char ** argv ){
   ...
   int rwidth = w.ws_col;
   int rheight = w.ws_row;

   unsigned char *pixels = ( unsigned char * )calloc( rheight * rwidth * 3, sizeof( unsigned char ) );
   ...
   stbir_resize_uint8( image, width, height,0,
                       pixels, rwidth, rheight,0,
                       channels);

   print_image( pixels , rheight, rwidth );
